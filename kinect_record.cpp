...
int Kinct::onePicture(const int i, k4a_capture_t sensor_capture, float(&fAngelUsing)[ANGLE_NUM], k4abt_tracker_t& tracker, uint64_t& timeStamp, char name[80])
{
	//打开日志
	FILE* logFile = nullptr;
	VERIFY(fopen_s(&logFile, name, "a"), "logFIle create faild");
	...
	if (queue_capture_result == K4A_WAIT_RESULT_TIMEOUT)// && queue_capture_result1 == K4A_WAIT_RESULT_TIMEOUT)
	{
		...
		PRINTFORLOG(logFile, "Error! Add capture to tracker process queue timeout!\n");
	}
	else if (queue_capture_result == K4A_WAIT_RESULT_FAILED)// && queue_capture_result1 == K4A_WAIT_RESULT_FAILED)
	{
		PRINTFORLOG(logFile, "Error! Add capture to tracker process queue failed!\n");
	}
	else
	{
		...
		if (pop_frame_result == K4A_WAIT_RESULT_SUCCEEDED)
		{
			...
			{
				PRINTFORLOG(logFile,"have joints\n");
				////***************求角度*******************
			...
			{
				PRINTFORLOG(logFile,"Get body skeleton failed!!\n");
			}
		...
		{
			//  It should never hit timeout when K4A_WAIT_INFINITE is set.
			PRINTFORLOG(logFile,"Error! Pop body frame result timeout!\n");
			return 1;
		}
		else
		{
			PRINTFORLOG(logFile,"Pop body frame result failed!\n");
			return 1;
		}
	}
	bOnePicture = false;
	PRINTFORLOG(logFile,"have recorded\n");
	...
	float(&fAngelUsing)[ANGLE_NUM], uint64_t& timeStamp)  //普通的函数，用来执行线程
{
	...
	// 把 now 转换为字符串形式
	char* dt = ctime(&now);
	_itoa_s(i, i2s, 10);//序号deviceindex变，文本tmp
	strcpy(name, ".\\logFile-"); //前面的filename_
	strftime(name, sizeof(name), "%m_%d_%H_%M_%S_", localtime(&now));
	strcat(name, i2s); //文件序号
	strcat(name, ".csv"); //文件后缀名
	...
	while (1)
	{
		if (k4a_device_get_capture(dev, &sensor_capture, K4A_WAIT_INFINITE) == K4A_WAIT_RESULT_SUCCEEDED)
		{
			...
			{
				capTidFlag = true;
				capTid = thread(&Kinct::onePicture, this, i, sensor_capture, ref(fAngelUsing), ref(tracker), ref(timeStamp), name);
			}
